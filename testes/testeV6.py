...
# F = (P * D) / W
# F = (128 * 30) / 8.5 = 451.76
# P = Pixels | D = Distancia(cm) | W = Largura real
...
def main():
    ...
    while True:
        ...
        if found:
            ...
            if(last_known_error_x < 20 and last_known_error_y < 20):
        ...
        # Se começou a descida:
        if(mission_status == 'INICIANDO DESCIDA'):
            if(last_known_distance < aprox_final_alt):
                    mission_status = 'APROX. FINAL'
                    aprox_final_tempo = time.time()
                    if(last_known_distance < 20):
                        mission_status = 'Pouso realizado'

        if(mission_status == 'APROX. FINAL'):
            if(time.time() - aprox_final_tempo > 2):
                mission_status = 'POUSO REALIZADO'
